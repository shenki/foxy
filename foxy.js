...
function continueInit() {
    ...
    if (geolocationSupport) {
        ...
        var opts = {enableHighAccuracy: true,
            timeout: 600000,
            maximumAge: 1000000};
        navigator.geolocation.watchPosition(showPosition, error, opts);
    }
...
function onDeviceOrientationChange(event) {
    if (event.webkitCompassHeading != undefined) {
        ...
        document.control_panel.bearingsource.value = "webkit";
    }
    else if (event.alpha != null) {
        currentHeading = 270 - event.alpha;
        if (currentHeading < 0) {
            currentHeading = currentHeading + 360;
        }
        //document.control_panel.alpha.value = event.alpha;
        //document.control_panel.bearingsource.value = "alpha";
    }
